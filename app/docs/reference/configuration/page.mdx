...
Each name is resolved the same way `environment.secrets` names are - from
...
Classic (non-buildx) `docker build` only understands `--secret` when
...
services:
  monitoring:
    ...
    servers:
      - server1
    network_mode: bridge   # bridge (default), service:<other-service-name>, or host
```

`network_mode: none` is rejected by validation: every service needs a
...
services:
  app:
    ...
    network_mode: host
    ports:
      - "8080"   # what the service listens on; never rendered as -p
```

`ports:` accepts at most one entry, a bare container-side port number only
