...
- **Health-Gated Rollouts** - For rolling services, the old version keeps serving until the new one passes its health check. `stop_first` services use a brief stop-then-start window instead; a service with a fixed host-port binding needs `stop_first` for the same reason, or its deploys fail outright instead of degrading gracefully.
...
- **Build Secrets** - Mount tokens and credentials into a build via `--secret` instead of `--build-arg`, so they never land in image layers or metadata.
...
- **Project Bridge Networking** - Give each deployment a dynamic address on
  the project bridge.
...
- **Host Networking** - Run a service on the host's own network namespace (`network_mode: host`) instead of the project bridge, for containers that need direct host-level ports.
...
- **Interactive Remote Execution** - Run a command with a PTY on one selected server via `jiji server exec`.
...
- **Audit Trail** - Best-effort, per-server operation history with filtering.
...
- **Server Upgrade** - Bring `jiji-agent` and `jiji-proxy` on each server in line with the installed `jiji` CLI, per host and per component, without downgrading.
