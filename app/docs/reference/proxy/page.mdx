export const metadata = {
  description: 'Configure jiji-proxy routing, domains, TLS certificates, health checks, and zero-downtime traffic switching with Jiji.',
...
public HTTP and HTTPS entry point for proxied services. Jiji installs and
manages the proxy container, translates each service's `proxy:` configuration
into a route definition, and jiji-proxy itself continuously discovers that
route's backends over DNS rather than being told an address on every deploy.
...
A jiji-proxy route is identified by the `hosts`/`path_prefix` you configure,
not a generated name. jiji-proxy pushes no explicit backend address into the
route at all: instead it continuously resolves the service's **aggregate**
DNS name,
...
| `deploy_timeout` | Maximum time allowed for both the pre-activation gate and proxy activation; defaults to `60s` |
...
shows every route currently configured on that host's jiji-proxy; `route
status --host <host>` shows the backend addresses it has actually discovered
for that route and whether each one currently passes its health check.
