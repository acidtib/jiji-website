...
flowchart TB
  ...
  accDescr: Internet traffic reaches Kamal Proxy on ports 80 and 443, then routes directly to the current Active deployment's dynamically leased address.
  internet([Internet])
  proxy["Kamal Proxy"]
  active["Active deployment<br/>dynamically leased address"]

  internet -->|"TCP 80 / 443"| proxy
  proxy -->|"HTTP routing"| active
...
| `deploy_timeout` | Maximum time allowed for proxy activation; defaults to `60s` |

A `cmd` check works both for Jiji's own pre-activation gate (the check that
runs before a candidate is admitted, described below) and for Kamal Proxy's
own *ongoing* route health check, because each host's Kamal Proxy route
only ever contains addresses running on that same host (see "Route
Identity" above) - `cmd` execs into a container, which only ever works for
...
3. Publishes the candidate Active in the replicated catalog.
4. Rebuilds the Kamal Proxy route on that host from its own local healthy
   Active replicas of the service. A host with no local replica has its
   route withdrawn rather than pointed at a replica on another host - Kamal
   Proxy does not load-balance across hosts.
5. Verifies that the route exists.
6. Drains the previous deployment and releases its lease.
...
previous Active catalog record and proxy route, then removes the failed candidate.
...
Routes point directly to the current Active deployment's leased address and
service port.

Because each host only routes to its own local replicas (see "Route
Identity" above), point public DNS only at servers that actually run the
service. If `replicas` is lower than the number of servers in the eligible
placement set (see [Configuration Reference](/docs/reference/configuration#replicas-and-placement)),
some of those servers may have no local replica at any given moment and no
route for the service at all - DNS pointed there gets no response rather
than a forwarded one. A dedicated load-balancer service is the intended way
to spread traffic across a server set larger than the running replica
count; Kamal Proxy does not do this itself.
