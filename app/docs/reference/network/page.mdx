...
container subnet and runs a per-project agent. Agents replicate membership,
desired placement, the service catalog, and DNS data directly over the mesh.
There is no central coordinator.

```mermaid
flowchart LR
  accTitle: Distributed Jiji service network
  accDescr: Two project agents replicate membership, desired placement, and the service catalog over WireGuard. Each agent answers DNS from the same converged catalog, while service containers use dynamically leased addresses.

  subgraph a["node-a"]
    agentA["Jiji agent<br/>DNS + catalog"]
    appA["web replica<br/>dynamic lease"]
    proxyA["kamal-proxy<br/>all healthy targets"]
  end

  subgraph b["node-b"]
    agentB["Jiji agent<br/>DNS + catalog"]
    appB["web replica<br/>dynamic lease"]
    proxyB["kamal-proxy<br/>all healthy targets"]
  end

  agentA <-->|"WireGuard replication"| agentB
  appA --> agentA
  appB --> agentB
  proxyA --> appA
  proxyA --> appB
...
`project:`. Multiple projects can share a physical server. Kamal-proxy is the
one host-global component and is attached to each project bridge that owns
routes on that host.

Default address pools are shared across projects. Project hashing makes normal
collisions unlikely, but it is not a global allocator. Configure explicit
non-overlapping CIDRs when independent projects must share hosts with a hard
guarantee.
...
- bridge gateway, agent DNS, and kamal-proxy infrastructure addresses.
...
Membership records are authority-signed. Catalog and desired-placement records
are signed by active member nodes. Agents exchange durable operations directly
with authenticated peers and converge after duplicate, reordered, or delayed
delivery.

A filtered `jiji server setup -H new-node` connects the new node plus one
reachable seed for enrollment. Existing offline peers are not required. Adding
or removing actual servers changes the mesh; deploying or scaling services
does not.
...
`services.*.servers` is the eligible host set. `replicas` is the total desired
count, not a count per host. `placement: spread` balances replicas;
`placement: packed` chooses hosts deterministically.

`jiji service scale` writes signed replicated desired placement before it
