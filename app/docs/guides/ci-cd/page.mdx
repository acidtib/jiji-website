...
`jiji deploy` automatically acquires the project lock across every
configured server before network reconciliation and releases it when the
command finishes. A normal pipeline only needs the deploy step:
...
Use `jiji lock acquire` separately when you need a maintenance window that
blocks deploy, service restart, and service rollback operations.
```
