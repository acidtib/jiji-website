...
flowchart TB
  ...
  candidate["Candidate deployment<br/>gets an address lease"]
  health{"Healthy?"}
  catalog["Publish Active<br/>in replicated catalog"]
  routes["Reconcile DNS<br/>and proxy targets"]
  ...
  health -->|"yes"| catalog --> routes --> cleanup
  health -->|"no"| rollback
...
address, never through DNS or kamal-proxy, so a healthy result really
...
Scale commits signed desired placement first, then adds or retires replicas.
