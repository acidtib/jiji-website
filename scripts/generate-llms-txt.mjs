...
const siteUrl = (process.env.SITE_URL || "https://jiji.run").replace(/\/$/, "");
...
function nestHeadings(markdown, wrapperLevel) {
  let fence = null;
...
function excerpt(markdown, maxLen = 220) {
...
const introTitle = pageTitle(introSource, metaTitle(topMeta.index, "Introduction"));
...
const llms = [
  ...
  "## Documentation",
  "",
  `- [${introTitle}](${siteUrl}/docs): ${introSummary}`,
];
...
writeFileSync(path.join(root, "public/llms.txt"), llms.join("\n") + "\n");
writeFileSync(path.join(root, "public/llms-full.txt"), full.join("\n") + "\n");
writeFileSync(path.join(root, "public/sitemap.xml"), urlSet(["/"]));
writeFileSync(path.join(root, "public/docs/sitemap.xml"), urlSet(docsRoutes));
writeFileSync(
