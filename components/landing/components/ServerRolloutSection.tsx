      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {TOP_FEATURE_CARDS.map((card) => (
            <div
              key={card.id}
              className="group relative bg-[#0a0d0a] border border-zinc-800 hover:border-lime-500/50 rounded-xl p-6 transition-all duration-300 flex flex-col justify-between hover:shadow-xl hover:shadow-lime-950/30"
            >
              {/* Badge & Icon Header */}
              <div>
              ...
              {/* Tech Specs bullets */}
              <div className="pt-4 border-t border-zinc-800/80 font-mono text-[11px] text-zinc-500 space-y-1.5">
                {card.techSpecs.map((spec, idx) => (
